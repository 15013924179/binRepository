...
@Component
@Slf4j
public class MalaixiyaBookingCrawsService {
    @Autowired
    ...
    public void crawsListPage() throws Exception {
        ...
        List<Document> documents = mongoTemplate.find(new Query(Criteria.where("is_craw").is(false)), Document.class, "malaixiya_booking_hotel_city");
        ...
        for (Document area : documents) {
            ...
            while (true) {
                ...
                for (WebElement webElement : elements) {
                    ...
                    try {
                        ...
                        boolean exists = mongoTemplate.exists(new Query(Criteria.where("url").is(oldUrl)), Document.class, "malaixiya_booking_hotel");
                        if (exists) {
                            log.info("该列表项已经爬取过");
                            continue;
                        }
                    ...
                    mongoTemplate.save(document, "malaixiya_booking_hotel");
            ...
            mongoTemplate.save(area, "malaixiya_booking_hotel_city");
    ...
    public void crawDetailPage () throws Exception{
        WebDriver webDriver = MySeleniumUtils.getWebDriver();
        log.info("开始爬取详情页");
        List<Document> documents = mongoTemplate.find(new Query(Criteria.where("is_craw").is(false)), Document.class, "malaixiya_booking_hotel");
